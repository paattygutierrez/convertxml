...
def processar_nfe_por_item(xml_path, ns):
    ...
    try:
        ...
        for det in det_list:
            ...
            descricao_item = prod.find('ns:xProd', ns)
            descricao_item_val = descricao_item.text if descricao_item is not None else ""

            ncm = prod.find('ns:NCM', ns)
            ncm_val = ncm.text if ncm is not None else ""
...
def _remover_namespace(root):
    """
    Remove o namespace de todos os elementos de uma árvore XML já carregada,
    permitindo localizar tags (ex: <toma>, <serv>) sem precisar conhecer
    o namespace/prefixo utilizado pelo layout de NFSe (varia por município/
    padrão nacional).
    """
...
def _texto(elemento, tag, contexto=None):
    """Busca 'tag' dentro de 'elemento' (ou de 'contexto' se elemento for None) e retorna o texto."""
    alvo = elemento if elemento is not None else contexto
    if alvo is None:
        return ""
    achado = alvo.find(f'.//{tag}')
    return achado.text if achado is not None and achado.text is not None else ""

def processar_servico(xml_path):
    """
    Processa um arquivo XML de NFSe/DPS (Nota Fiscal de Serviço Eletrônica /
    Declaração de Prestação de Serviços - layout nacional), extraindo os
    dados do prestador, do grupo <toma> (tomador do serviço), do serviço
    prestado e dos tributos (ISS, IRRF, CSLL, IBS, CBS).

    Observação: numa DPS recém-enviada (ainda não processada pela
    prefeitura), os valores de ISS, IBS e CBS costumam não vir calculados
    no próprio arquivo - apenas indicadores de tributação (tribISSQN,
    CST/cClassTrib do grupo IBSCBS). Quando o XML for de uma NFSe já
    autorizada, esses valores tendem a aparecer preenchidos.
    """
    try:
        tree = ET.parse(xml_path)
        root = _remover_namespace(tree.getroot())

        # Identificação da DPS/NFSe
        numero_nfse = _texto(root, 'nNFSe') or _texto(root, 'nDPS')
        data_emissao = _texto(root, 'dhEmi') or _texto(root, 'dhProc')
        chave_acesso = _texto(root, 'chNFSe') or _texto(root, 'chDFe')

        # Prestador do serviço
        prest = root.find('.//prest')
        cnpj_prestador = _texto(prest, 'CNPJ')
        nome_prestador = _texto(prest, 'xNome')  # nem sempre presente na DPS

        # Grupo <toma> - tomador do serviço
        toma = root.find('.//toma')
        cnpj_toma = _texto(toma, 'CNPJ')
        xnome_toma = _texto(toma, 'xNome')

        # Dados do serviço prestado (serv > cServ > cTribNac / xDescServ)
        cServ = root.find('.//cServ')
        cTribNac = _texto(cServ, 'cTribNac')
        xDescServ = _texto(cServ, 'xDescServ')

        # Valor do serviço: valores > vServPrest > vServ
        vServ = _texto(root, 'vServPrest/vServ') or _texto(root, 'vServ')

        # ISS: normalmente só o indicador de tributação vem na DPS;
        # tenta também nomes de tag usados quando o valor já vem calculado.
        vISS = (_texto(root, 'vISS') or _texto(root, 'vISSQN')
                or _texto(root, 'vValorIssqn'))
        tribISSQN = _texto(root, 'tribISSQN')  # 1 = tributável, indicador
        tpRetISSQN = _texto(root, 'tpRetISSQN')  # indica se há retenção

        # IRRF e CSLL retidos: trib > tribFed > vRetIRRF / vRetCSLL
        vIRRF = _texto(root, 'vRetIRRF') or _texto(root, 'vIRRF')
        vCSLL = _texto(root, 'vRetCSLL') or _texto(root, 'vCSLL')

        # IBS/CBS: grupo <IBSCBS>. Valor calculado (vIBS/vCBS) só existe
        # em NFSe já processada; CST/cClassTrib vêm mesmo na DPS.
        gibscbs = root.find('.//gIBSCBS')
        vIBS = _texto(root, 'vIBS')
        vCBS = _texto(root, 'vCBS')
        cst_ibscbs = _texto(gibscbs, 'CST')
        cclasstrib_ibscbs = _texto(gibscbs, 'cClassTrib')

        return [{
            "Número NFSe/DPS": numero_nfse,
            "Data de Emissão": data_emissao,
            "CNPJ Prestador": cnpj_prestador,
            "Prestador": nome_prestador,
            "CNPJ Tomador": cnpj_toma,
            "Tomador": xnome_toma,
            "cTribNac": cTribNac,
            "xDescServ": xDescServ,
            "vServ": vServ,
            "ISS": vISS,
            "Tributável ISSQN": tribISSQN,
            "Tipo Retenção ISSQN": tpRetISSQN,
            "IRRF": vIRRF,
            "CSLL": vCSLL,
            "IBS": vIBS,
            "CBS": vCBS,
            "CST IBS/CBS": cst_ibscbs,
            "cClassTrib IBS/CBS": cclasstrib_ibscbs,
            "Chave de Acesso": chave_acesso,
        }]
    except Exception as e:
        st.error(f"Erro ao analisar o arquivo XML: {os.path.basename(xml_path)}. Erro: {e}")
...
def main():
    ...
    tipo_doc = st.radio("Tipo de Documento:", ["NFe", "CTe", "Serviço"])
                ...
                if not xml_files:
                    ...
                    for i, xml_file in enumerate(xml_files):
                        ...
                        if tipo_doc == "NFe":
                            ...
                            dados_totais.extend(processar_servico(xml_file))
