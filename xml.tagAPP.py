...
```
 ...
 # O IBS pode estar em diferentes estruturas dependendo da versão do schema
 ibs = imposto.find('.//ns:IBS', ns)
 ibs_valor = None
 ibs_cst = None
 if ibs is not None:
 # Tenta encontrar o valor e CST dentro de subgrupos do IBS
 for subtag in ibs:
 v = subtag.find('ns:vIBS', ns)
 c = subtag.find('ns:CST', ns)
 if v is not None:
 ibs_valor = v
 if c is not None:
 ibs_cst = c
 # Fallback direto
 if ibs_valor is None:
 ibs_valor = ibs.find('.//ns:vIBS', ns)
 if ibs_cst is None:
 ibs_cst = ibs.find('.//ns:CST', ns)

 # ---- CBS (Contribuição sobre Bens e Serviços) ----
 cbs = imposto.find('.//ns:CBS', ns)
 cbs_valor = None
 cbs_cst = None
 if cbs is not None:
 for subtag in cbs:
 v = subtag.find('ns:vCBS', ns)
 c = subtag.find('ns:CST', ns)
 if v is not None:
 cbs_valor = v
 if c is not None:
 cbs_cst = c
 # Fallback direto
 if cbs_valor is None:
 cbs_valor = cbs.find('.//ns:vCBS', ns)
 if cbs_cst is None:
 cbs_cst = cbs.find('.//ns:CST', ns)
...
“””
Processa um arquivo XML de NFe extraindo dados por cabeçalho.
“””
try:
tree = ET.parse(xml_path)
root = tree.getroot()
...
```
 ...
 # ---- IBS e CBS totais (nível ICMSTot ou IBSCBSTot) ----
