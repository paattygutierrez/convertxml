...
def processar_servico(xml_path):
 """
 Processa um arquivo XML de NFSe (Nota Fiscal de Serviço Eletrônica),
 extraindo os dados do prestador, do grupo <toma> (tomador do serviço),
 do serviço prestado e dos tributos (ISS, IRRF, CSLL, IBS, CBS).
 """
 try:
 tree = ET.parse(xml_path)
 ...
 numero_nfse = _texto(root, 'nNFSe')
 if not numero_nfse:
 numero_nfse = _texto(root, 'nDPS')
 ...
 cnpj_prestador = _texto(prest, 'CNPJ', root)
 nome_prestador = _texto(prest, 'xNome', root)
 ...
 serv = root.find('.//serv')
 cTribNac = _texto(serv, 'cTribNac', root)
 xDescServ = _texto(serv, 'xDescServ', root)
 vServ = _texto(serv, 'vServ', root)

 # Tributos
 vISS = _texto(root, 'vISS')
 vIRRF = _texto(root, 'vIRRF')
 vCSLL = _texto(root, 'vCSLL')
